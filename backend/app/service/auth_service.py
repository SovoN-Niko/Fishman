...
class AuthService:
    ...
    async def register_service(register: RegisterSchema):
        ...
        _users_role = UsersRole(
            users_id=_users_id, role_id=_role.id, modified_at=datetime.utcnow()
        )
...
async def generate_role():
    ...
    if not _role:
        ...
        roles = [
            Role(id=str(uuid4()), role_name="admin", modified_at=current_time),
            Role(id=str(uuid4()), role_name="user", modified_at=current_time),
        ]
        await RoleRepository.create_list(roles)
